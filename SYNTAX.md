...
## Compilation Options
Within the file [`flit.h`](flitdb/flit.h), you can limit the total operable size of the database, by setting the compilation value of `FLITDB_SIZING_MODE` to one of three different values; this definition of a sizing mode is put in place to further limit the amount of data that FlitDB will have access to. Additionally, you also have the option to allow memory mapping the database file, this results in a faster overall "read" time as the reading of the database can be accomplished from reading the mapped memory database file, instead of reading from *disk* each time; this feature is currently only supported when the database is opened the [`FLITDB_READONLY`](#access-flags) option set, and the database is less than - or equal to - 20 megabytes in size; to use this feature you must also have the value of `FLITDB_MMAP_ALLOWED` to `1` [`flit.h`](flitdb/flit.h) (default value - is enabled by default).
### Sizing mode
